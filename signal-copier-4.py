...
SMC_TZ_ID = -1002469668344            # SMZ Trade Zone (PAID)
...
VIP_CHANNELS = [SMC_GOLD_ID]
provider_name = ["King SMC", "SMC Gold Expert", "SMC TZ", "Forex Gold Expert", "CRT Signals Team", "SMC XAUUSD(GOLD)","SMC Gold Special"]
...
FULLY_CLOSE_RE = re.compile(r'\bfully close\b', re.IGNORECASE)
...
async def process_provider_message(event):
    ...
    if event.chat.id == 1914952549:
        ...
        if USE_BE_RE.search(text):
            ...
            if not sym:
                replied = await event.get_reply_message()
                sym = extract_symbol_from_text(replied.text)
    ...
    if event.chat.id == 2095470635:
        ...
        if FULLY_CLOSE_RE.search(text):
            close_positions_for_symbol("XAUUSD-STDc", channel_id_provider_name_map[event.chat.id])

        if USE_BE_RE.search(text):
            is_management = True
            move_sl_to_entry("XAUUSD-STDc", channel_id_provider_name_map[event.chat.id])
            # return
        parsed = parse_signal(text)
    # signal test
    ...
    def _place_one(provider: str, _tp) -> int | None:
        ticket = None
        try:
            vol = None
    ...
    """
    if event.chat.id == 2469668344:
        ticket = _place_one(channel_id_provider_name_map[event.chat.id], tp)
    if event.chat.id == 1914952549:
        ticket = _place_one(channel_id_provider_name_map[event.chat.id], tp)
    if event.chat.id == 2095470635:
        ticket = _place_one(
            channel_id_provider_name_map[event.chat.id],
            tp
        )
        tick = mt5.symbol_info_tick(symbol)
        if tick is None:
        ...
        ref_price = entry if entry is not None else price_for_order

        # Use ask for buys, bid for sells
        current_price = tick.ask if tp > ref_price else tick.bid
        if tp > ref_price:
            ticket = _place_one(
                channel_id_provider_name_map[event.chat.id],
                # current_price + 10
...
async def main():
    ...
    print(f"Listening to channel(s): {provider_name[1]}")
